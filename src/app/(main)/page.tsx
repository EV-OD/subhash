...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <section className="w-full py-24 md:py-32 lg:py-40 bg-background text-text-heading relative overflow-hidden">
        {/* Global Grid Texture */}
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-muted/50 via-background to-background opacity-50"></div>
        <div className="container relative z-10 px-4 md:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              <div className="inline-flex items-center rounded-full border border-border bg-muted/50 px-3 py-1 text-sm text-text-description backdrop-blur-sm">
                <Scale className="mr-2 h-4 w-4" />
                <span>Advocate Subhash Lamichhane</span>
              </div>
              <h1 className="text-4xl font-bold tracking-tight sm:text-5xl md:text-6xl lg:text-7xl font-serif text-text-heading">
                Legal Insight & <br />
                <span>Academic Excellence</span>
              </h1>
              <p className="text-lg md:text-xl text-text-description max-w-[600px] leading-relaxed">
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                <Button asChild size="lg" className="font-medium text-text-button">
                  <Link href="/blog">
                    Explore Resources <ArrowRight className="ml-2 h-4 w-4" />
                  </Link>
                </Button>
                <Button asChild size="lg" variant="outline" className="border-border text-text-heading hover:bg-muted">
      ...
      <section className="w-full py-24 bg-muted/30 text-text-heading relative overflow-hidden">
        {/* Background Elements */}
        ...
        <div className="container relative z-10 px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            ...
            <Button asChild variant="outline" className="border-border text-text-description hover:bg-background hover:text-text-heading hidden md:flex">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-[280px]">
            {practiceAreas.map((area, index) => (
              <div 
                key={area.slug} 
                className={`group relative overflow-hidden rounded-3xl bg-card border border-border p-8 transition-all duration-500 hover:border-primary/20 hover:shadow-xl ${
              ...
              >
                ...
                <area.icon className="absolute -bottom-8 -right-8 w-48 h-48 text-muted/40 group-hover:text-muted/60 transition-colors duration-500 rotate-[-15deg]" />

                <div className="relative z-10 flex flex-col h-full justify-between">
                  <div className="w-12 h-12 rounded-2xl bg-muted/50 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 border border-border group-hover:border-primary/50">
                    <area.icon className="h-6 w-6 text-text-description group-hover:text-primary transition-colors" />
                  </div>
                  
                  <div className="space-y-3">
                    <h3 className="text-2xl font-bold font-serif text-text-heading group-hover:text-primary transition-colors">{area.title}</h3>
                    <p className="text-text-description text-sm leading-relaxed max-w-[90%] group-hover:text-text-heading transition-colors">
                      {area.description}
                    </p>
                  </div>
                </div>
            ...
            {allContent.map((post) => (
              <Link key={post.slug} href={`/${post.type}/${post.slug}`} className="group cursor-pointer space-y-3 block">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-text-description line-clamp-3">
                    {post.entry.excerpt}
                  </p>
                  <div className="pt-2 text-sm font-medium underline decoration-border underline-offset-4 group-hover:decoration-primary transition-all text-text-heading">
                    Read More
                  </div>
                </div>
              </Link>
      ...
      <section className="w-full py-24 bg-slate-950 dark">
        <div className="container px-4 md:px-6 text-center">
          <div className="max-w-2xl mx-auto space-y-6">
            <h2 className="text-3xl font-bold tracking-tight sm:text-4xl font-serif text-text-heading">
            ...
            <Button asChild size="lg" className="bg-foreground text-background hover:bg-muted mt-4">
